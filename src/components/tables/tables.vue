  ...
  methods: {
    ...
    surportHandle (item) {
      ...
      options.forEach(item => {
        ...
        if (item === 'reset' && !this.access_reset) return false
        if (item === 'grant' && !this.access_grant) return false
    ...
    supportStatus (item) {
      ...
      item.render = (h, params) => {
        ...
        } else if (params.row.status === '待上报' || params.row.status === '待审核') {
          dom = [
            h('span', {
              class: 'tableStatus',
              style: {
        ...
        } else if (params.row.status === '退回' || params.row.status === '冻结') {
          dom = [
            h('span', {
              class: 'tableStatus',
              style: {
    ...
    Status (item) {
      let dom
      item.render = (h, params) => {
        if (params.row.temporary === '1') {
          dom = [
            h('span', {}, '完成')
          ]
        } else if (params.row.temporary === '0') {
          dom = [
            h('span', {}, '进行中')
          ]
        }
